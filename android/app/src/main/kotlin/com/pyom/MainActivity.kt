...
class MainActivity : FlutterActivity() {
    ...
    // ── proot binary location ─────────────────────────────────────────────
    // AGP 8.x with jniLibs.useLegacyPackaging=true extracts to nativeLibraryDir.
    // Fallback chain handles devices where extraction path differs.
    private val prootBin: File get() {
        val name = "libproot.so"

        // Primary: nativeLibraryDir (standard location after APK install)
        val primary = File(applicationInfo.nativeLibraryDir, name)
        if (primary.exists() && primary.length() > 0) return primary

        // Fallback 1: manually extracted copy in filesDir/bin/
        val manual = File(filesDir, "bin/$name")
        if (manual.exists() && manual.length() > 0) return manual

        // Fallback 2: common alternative nativeLibraryDir paths
        val altDirs = listOf(
            "/data/app/${packageName}-1/lib/arm64",
            "/data/app/${packageName}-2/lib/arm64",
            filesDir.parent + "/lib/arm64",
        )
        for (dir in altDirs) {
            val f = File(dir, name)
        ...
        // Return primary path even if missing (error will be shown to user)
        return primary
    }

    // Extract proot from APK assets if nativeLibraryDir is empty (AGP 8 quirk)
    private fun ensureProotExtracted() {
        if (prootBin.exists() && prootBin.length() > 100_000L) return

        val dest = File(filesDir, "bin/libproot.so")
        dest.parentFile?.mkdirs()

        // Try to extract from APK zip directly
        try {
            val apkFile = packageManager.getApplicationInfo(packageName, 0).sourceDir
            ...
            // APK extraction failed — will show error to user
        }
    }
    ...
    private fun setupEnvironment(distro: String, envId: String, result: MethodChannel.Result) {
        try {
            ...
            // Try to extract proot from APK if nativeLibraryDir is empty (AGP 8.x quirk)
            sendProgress("Checking proot binary…", 0.02)
            ensureProotExtracted()

            sendProgress("Checking proot binary…", 0.03)
            if (!prootBin.exists() || prootBin.length() < 100_000L) {
                val nativeDir = applicationInfo.nativeLibraryDir
                val manualDir = File(filesDir, "bin").absolutePath
                val apkPath   = packageManager.getApplicationInfo(packageName, 0).sourceDir
                mainHandler.post {
                    result.error("SETUP_ERROR",
                        "libproot.so not found or too small.\n\n" +
                        "Debug info:\n" +
                        "  nativeLibraryDir: $nativeDir\n" +
                        "  nativeLibraryDir exists: ${File(nativeDir).exists()}\n" +
                        "  files in nativeDir: ${File(nativeDir).listFiles()?.map { it.name } ?: emptyList<String>()}\n" +
                        "  manual path: $manualDir/libproot.so exists: ${File(manualDir, "libproot.so").exists()}\n" +
                        "  APK: $apkPath\n\n" +
                        "Fix: Rebuild APK with jniLibs.useLegacyPackaging=true in build.gradle", null)
                }
                return
            }
